    ...
    while change == True and np.shape(A)[1] > 0:
        ...
        for i in range(np.shape(A)[1]): #for each column in the binary matrix A
            ...
            if len(RN_i) == 1:  #if its the single RN of column i
                ...
                print(col_del)
                row_del.append(RN_i)
        #col_del = np.array(col_del)#.reshape((1,)
        #col_del = np.squeeze(np.asarray(col_del))

        #col_del = np.array(col_del).flatten()
        #row_del = np.array(row_del).flatten()
        print(col_del)
        print(row_del)
        #num1 = int(input())
        print(np.shape(A))
        ...
        for j in range(np.shape(A)[0]):
            ...
            A_dif = np.delete(A_dif, j_no, 0)  #delate itself substract and the already delated rows
            #print(A_dif == -1)
            #print((A_dif == -1)*1)

            boolarr = np.sum((A_dif == -1)*1,axis = 1) #check if row j can be delated
            boolarrp = np.prod(boolarr)
            #boolarrp = np.min(boolarr)
            #print(boolarrp)
            #num1 = int(input())
            #print(A_dif == -1)
            #print((A_dif == -1)*1)
