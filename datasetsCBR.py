...
def load_credita(i):
    train_path = os.path.join('datasetsCBR', 'credit-a', f'satimage.fold.00000{str(i)}.train.arff')
    test_path = os.path.join('datasetsCBR', 'credit-a', f'satimage.fold.00000{str(i)}.test.arff')

    df_train = loadarff(train_path)
    df_test = loadarff(test_path)
    ...
    X_test[num_cols] = mm_scaler.transform(X_train[num_cols])
